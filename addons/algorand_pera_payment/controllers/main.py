...
class PeraPaymentController(http.Controller):
    ...
    def algorand_pera_form(self, **kwargs):
        ...
        _logger.info(
            "[Algorand][algorand_pera_form] CALLED with kwargs=%s",
            list(kwargs.keys()),
        )
        ...
        _logger.info(
            "[Algorand][algorand_pera_form] RETURNING payment form for "
            "tx_id=%s reference=%s",
            tx_id,
            reference,
        )
    ...
    @http.route(
        "/payment/algorand_pera/process", type="json", auth="public", csrf=False
    )
    def algorand_pera_process(self, **kwargs):
        """Process the Algorand payment after blockchain confirmation.

        ENTRY POINT: Called by frontend after blockchain transaction is
        broadcast
        ...
        """
        _logger.info(
            "[Algorand][algorand_pera_process] "
            "==================== CALLED ===================="
        )
        ...
        _logger.info(
            "[Algorand][algorand_pera_process] Payload: tx_id=%s tx_hash=%s "
            "sender=%s error=%s",
            tx_id,
            tx_hash,
            sender_address,
        ...
        if error_message:
            if "overspend" in error_message.lower():
                return {
                    "error": True,
                    "message": (
                        "Insufficient funds. Please add more ALGO to your wallet "
                ...
                return {
                    "error": True,
                    "message": (
                        "Payment failed. Please try again or contact support if "
        ...
        try:
            _logger.info(
                "[Algorand][process] Tx record: reference=%s amount=%s "
                "currency=%s provider=%s",
                tx.reference,
                tx.amount,
                tx.currency_id and tx.currency_id.name,
                tx.provider_code,
            )
        ...
        _logger.info(
            "[Algorand][process] Transaction %s monitored for /payment/status", tx.id
        )

        # Note: Accounting entries (account.payment, invoices) are NOT
        # created here. They are handled by Odoo's post-processing cron and
        # provider settings
        ...
        if so_id:
            ...
            if order and order.state in ("draft", "sent"):
                try:
                    ...
                    order.message_post(
                        body=f"Algorand payment confirmed. tx_id={tx_hash}"
                    )
                    _logger.info(
                        "[Algorand] Order %s confirmed successfully", order.name
                    )
                except Exception as e:
                    _logger.warning("[Algorand] Order confirmation failed: %s", e)

        # Cart clearing is handled automatically by Odoo's website_sale
        # module:
