...
class AlgorandPosController(http.Controller):
    ...
    def generate_qr_code(self, payment_method_id, amount, currency_id, order_ref):
        ...
        """
        _logger.info('[Algorand][POS] QR code request: method=%s amount=%s currency=%s ref=%s',
                     payment_method_id, amount, currency_id, order_ref)
        
        if not qrcode:
            return {
                'error': True,
                'message': 'QR code library not installed. Please install: pip install qrcode[pil]'
            }
        ...
        try:
            ...
            payment_data = payment_method.get_algorand_payment_data(amount, currency, order_ref)
            
            # Build Algorand payment URI for Pera Wallet
            # Format: algorand://pay?receiver=ADDR&amount=1000000&asset=ASSET_ID&note=ORDER_REF
            uri_params = {
            ...
            _logger.error('[Algorand][POS] QR code generation failed: %s', e, exc_info=True)
            return {
                'error': True,
    ...
    def verify_payment(self, tx_hash, payment_method_id, expected_amount, order_ref):
        ...
        """
        _logger.info('[Algorand][POS] Verify payment: tx=%s method=%s amount=%s ref=%s',
                     tx_hash, payment_method_id, expected_amount, order_ref)
        
        if not algod:
            return {
                'error': True,
                'message': 'Algorand SDK not installed. Please install: pip install py-algorand-sdk'
            }
        ...
        try:
            ...
            algod_url = request.env['ir.config_parameter'].sudo().get_param('algorand.node_url')
            algod_token = request.env['ir.config_parameter'].sudo().get_param('algorand.node_token', '')
            ...
            try:
                ...
                tx_info = algod_client.transaction_info(provider.algorand_merchant_address, tx_hash)
            ...
            _logger.info('[Algorand][POS] Transaction info: %s', json.dumps(tx_info, default=str))
            ...
            if abs(amount_microalgos - expected_microalgos) > 100:  # Allow 0.0001 difference
                return {
                    'error': True,
            ...
            _logger.error('[Algorand][POS] Payment verification failed: %s', e, exc_info=True)
            return {
                'error': True,
    ...
    def check_transaction_status(self, tx_hash, payment_method_id):
        ...
        """
        _logger.info('[Algorand][POS] Check transaction: tx=%s method=%s', tx_hash, payment_method_id)
        ...
        try:
            ...
            _logger.error('[Algorand][POS] Transaction status check failed: %s', e, exc_info=True)
            return {
                'error': True,
