...
class PaymentProvider(models.Model):
    ...
    def _algorand_get_inline_form_values(
        ...
        **kwargs,
        ...
        """Return a serialized JSON of the required values to render the inline
        form.

        Note: `self.ensure_one()`

        :param float amount: The amount in major units, to convert in minor
            units.
        :param res.currency currency: The currency of the transaction.
        :param int partner_id: The partner of the transaction, as a
            `res.partner` id.
        :param bool is_validation: Whether the operation is a validation.
        :param payment.method payment_method_sudo: The sudoed payment method
            record to which the inline form belongs.
        :return: The JSON serial of the required values to render the inline
            form.
        :rtype: str
        """
        self.ensure_one()
        ...
        currency_display_name = (
            "USDC" if use_usdc else (currency.name if currency else "ALGO")
        )

        # Use provider-level merchant address (no method-level override)
        merchant_address = self.algorand_merchant_address

        # Try to include a recent pending transaction id for this
        # partner/provider to help finalize
        tx_id_val = None
        try:
            if partner_id:
                tx = (
    ...
    def _send_payment_request(self, tx_sudo):
        ...
        # For Algorand Pera Wallet, we don't send a request to an external
        # provider. The payment is handled entirely on the frontend
            ...
            if provider.state in ("enabled", "test"):
                if not provider.algorand_merchant_address:
                    raise ValidationError(
                        _(
                            "Merchant Algorand Address is required to enable the "
                            "payment provider. Please configure your Algorand "
                            "wallet address in the provider settings."
                        )
                    )
                ...
                if len(address) != 58:
                    raise ValidationError(
                        _(
                            "Invalid Algorand address format. Algorand addresses "
    ...
    def _check_algorand_usdc_optin(self):
        ...
        try:
            ...
            _logger.error(
                ...
                (
                    self.algorand_merchant_address[:10] + "..."
                    if self.algorand_merchant_address
                    else "None"
                ),
                str(e),
            )
    ...
    def action_algorand_check_usdc_optin(self):
        """Check if merchant address is opted-in to USDC and display
        notification."""
        self.ensure_one()
        if self.code != "algorand_pera":
            return {"type": "ir.actions.act_window_close"}
        ...
        if not self.algorand_merchant_address:
            return {
                ...
                "params": {
                    "message": _(
                        "Please configure the Merchant Algorand Address first."
                    ),
                    "type": "warning",
                    "sticky": False,
                },
