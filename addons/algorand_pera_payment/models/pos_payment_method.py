...
class PosPaymentMethod(models.Model):
    ...
    def create_from_algorand_provider(self, provider):
        ...
        existing = self.search([("algorand_provider_id", "=", provider.id)], limit=1)

        if existing:
            _logger.info(
                "[Algorand][POS] Payment method already exists: %s", existing.name
            )
            return existing
        ...
        _logger.info(
            "[Algorand][POS] Created payment method: %s (ID: %s)",
            method.name,
            method.id,
        )
        return method
    ...
    def get_algorand_payment_data(self, amount, currency, order_ref):
        ...
        This method fetches the necessary data from the linked payment
        provider to generate a payment QR code that Pera Wallet can scan.
        ...
        if provider.state not in ("enabled", "test"):
            raise ValueError(
                f"Algorand provider is not enabled (state: {provider.state})"
            )
...
class PaymentProvider(models.Model):
    ...
    def write(self, vals):
        """When an Algorand provider is enabled, automatically create a POS
        payment method."""
        result = super().write(vals)

        # If provider is being enabled and it's Algorand, create POS payment
        # method
            ...
            for provider in self:
                if provider.code == "algorand_pera":
                    (
                        ...
                        .sudo()
                        .create_from_algorand_provider(provider)
                    )
    ...
    def create(self, vals_list):
        """When an Algorand provider is created and enabled, automatically
        create a POS payment method."""
        providers = super().create(vals_list)

        for provider in providers:
            if provider.code == "algorand_pera" and provider.state in (
                "enabled",
                "test",
            ):
                (
