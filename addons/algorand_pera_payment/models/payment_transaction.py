...
class PaymentTransaction(models.Model):
    ...
    # These methods override Odoo's standard payment flow to handle Algorand-specific data
    ...
    def _apply_updates(self, payment_data):
        ...
        Note: Post-processing (account.payment creation, invoice reconciliation)
        is handled by Odoo's standard cron job, not here. This prevents
        race conditions and database conflicts.
        """
        if self.provider_code != 'algorand_pera':
        ...
        if not tx_hash:
            _logger.error('[Algorand][tx] Missing tx_id in payment_data for ref=%s', self.reference)
            raise ValueError('Missing transaction ID in payment data')
            
        _logger.info('[Algorand][tx] _apply_updates called ref=%s txid=%s sender=%s state(before)=%s', self.reference, tx_hash, sender, self.state)
        ...
        self._set_done()
        _logger.info('[Algorand][tx] _apply_updates set done ref=%s txid=%s state(after)=%s', self.reference, tx_hash, self.state)
    ...
    def _search_by_reference(self, provider_code, payment_data):
        ...
        if reference:
            tx = self.search([('reference', '=', reference), ('provider_code', '=', 'algorand_pera')])
        else:
            tx = self
        if not tx:
            _logger.warning("No Algorand transaction found matching reference %s.", reference)
        return tx
    ...
    def _get_tx_from_notification_data(self, provider_code, notification_data):
        ...
        if reference:
            return self.search([('reference', '=', reference), ('provider_code', '=', 'algorand_pera')])
        return self.browse()
